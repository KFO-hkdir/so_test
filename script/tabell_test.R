...
pgrense_ordf_tabell <- datatable(
  ...
  width = "100%",
  options = list(
    dom = "ftip",                 # fewer controls; good for narrow layouts
    pageLength = 10,
    autoWidth = TRUE,
...
css_dt <- "
/* Make the widget fit a narrow iframe */
.html-widget { width: 100% !important; }

/* DT sizing tweaks for 640px-ish pages */
table.dataTable { 
  width: 100% !important; 
  font-size: 12px;
}
...
pgrense_ordf_tabell <- htmlwidgets::prependContent(
  pgrense_ordf_tabell,
  htmltools::tags$style(htmltools::HTML(css_dt))
)
...
pgrense_ord_tabell <- htmlwidgets::prependContent(
  pgrense_ord_tabell,
  htmltools::tags$style(htmltools::HTML(css_dt))
)
